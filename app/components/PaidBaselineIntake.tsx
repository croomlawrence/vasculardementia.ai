  ...
  return (
    <section className={`${compact ? "" : "border-4 border-black rounded-3xl p-6 md:p-8"} bg-white`} id="paid-intake">
      <div className="mb-6">
        ...
        <h2 className="text-3xl md:text-4xl font-semibold mb-4">Complete the routing details for your {memoryScreenPrice} cognitive baseline.</h2>
        ...
        {hasCheckoutSignal ? (
          ...
          <p className="border-2 border-black rounded-2xl p-4 bg-[#F8F4EA] font-semibold">
            If you have not completed checkout yet, start on the cognitive baseline page first. If Stripe sent you here after payment, continue below.
          </p>
        )}
      </div>
      ...
      <LeadForm
        ...
        title="Cognitive baseline intake"
        submitLabel="Submit Baseline Intake"
        context={context}
        compact
      />
      <p className="text-base text-black mt-4">
        Need to restart checkout? <Link href="/memory-screen" className="underline font-semibold">Return to the cognitive baseline page</Link>.
      </p>
    </section>
  );
}
